  ...
  return (
    <div className='bg-white p-5 rounded-lg my-1'>
      <div className='sm:flex-wrap flex justify-between'>
        ...
        <button className="text-black font-bold -mt-0 hover:text-red-800"> X </button>
      </div>

      {task.description ? (<p className="pt-2 text-black leading-4">{task.description}</p>) : null }
      ...
      <form className='pt-2'>
        <input className="accent-red-800" type="checkbox" id="" name="completed" value={task.isCompleted} />
      ...
      <div className='flex-wrap md:flex md:justify-between'>
        {task.assignee ? (
          <div className='pt-2'>
            ...
            <p className="border-2 border-red-800 rounded-2xl text-center align-middle 
             mt-4 w-16 px-2 bg-red-800 text-white mt-4 w-28 text-white">{task.assignee}</p>
          </div> 
        ) : null }
      
        <button className="border-2 border-black mt-5 md:mt-8 w-12 bg-black 
        text-white hover:cursor-pointer hover:text-white hover:bg-red-800 hover:border-red-800">edit</button> 
      </div>
    </div>
  )
}
